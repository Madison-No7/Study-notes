...
## 7)关于指定测试用例执行顺序问题
### a.为什么?
当一个项目中有多个测试用例文件时,Pytest的执行顺序是从上到下依次执行,文件里面的执行顺序也是从上到下依次执行.
![[Pasted image 20260831183132.png]]
但是有时候我们需要指定测试用例的执行顺序,否则测试不通过.
就比如关于博客系统的自动化接口测试,需要最先执行登录接口的测试用例,因为其他接口,需要登录接口返回的登录凭证(token)才能让接口正确响应,如果登录不最先执行,那登录测试之前的接口测试都不会通过,因为获取不到有效的token信息,自然就通过不了了.
因此指定测试执行顺序是十分重要的!
### b.怎么指定?
首先需要安装一个插件:
```python
...
```
执行结果:
![[Pasted image 20260831184311.png]]