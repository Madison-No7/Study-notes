...
```
对json字段加上严格的校验规则，限制json中age字段的值的范围在`0<=age<=100`。
...
**法二**：最终生成的测试报告为`html`文件，在当前路径下，可自行选择浏览器打开
...
```
### 5)指定测试用例的执行顺序
使用 `@pytest.mark.order(number)`装饰类或函数.
必须首先执行**登录接口测试用例(设计保存用户token)** 和**列表页接口测试用例(设计保存有效博客id信息)**,其他接口的测试用例测试顺序可以不指定.
### 6)生成测试报告并分析结果
生成测试报告:`pytest --alluredir allure-results`
查看测试报告:`allure generate allure-results -o allure-reports`
![[Pasted image 20260831190515.png]]
![[Pasted image 20260831190622.png]]
- **测试时间：**
测试执行时间从19:00:32 持续到 19:00:40，总共耗时 7 秒  612 毫秒。测试时间与测试用例数量成正比。用例数量越多，测试时间越长。通过优化测试脚本、并行执行和分布式测试环境，可以显著缩短测试时间。
- **测试用例总数：**
共有 53 个测试用例，**高测试用例总数**通常表示测试覆盖范围广，能够更全面地验证接口功能；**低测试用例总数**可能意味着测试覆盖不全面，存在遗漏的风险。
- **通过率：**
饼图显示了测试的通过率为 100%，这意味着所有 53 个测试用例都成功执行，没有失败的测试用例。通过率是衡量接口质量和测试效果的关键指标。在测试环境中，通过率应达到**95%以上**
### 7)测试脚本链接
